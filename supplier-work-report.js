(function(root,factory){
  ...
  function pageHtml(supplier,group,page,index,total,options){
    ...
    return `<section class="work-page"><header><h1>作業明細書</h1><span>${index+1} / ${total}</span></header>
      ...
      ${options.destination||options.contact?`<div class="delivery">${options.destination?`納品先 ${escape(options.destination)}　`:""}${options.contact?`連絡先 ${escape(options.contact)}`:""}</div>`:""}
      ${options.note?`<div class="instructions">${escape(options.note).replace(/\n/g,"<br>")}</div>`:""}
  ...
  const compactCss="body{line-height:1.25}header{padding:1mm 0}h1{line-height:1.2}.meta{padding:2mm 0}.lines th{padding:.6mm;line-height:1.15;height:8mm}.lines td{padding:.6mm;line-height:1.2}.boxes th{line-height:1.15;padding:.5mm}.boxes td{height:5.5mm}footer{margin-top:2mm}";
  ...
  function createWorkbook(ExcelJS,supplier,options={}){
    ...
    supplier.groups.forEach((group,groupIndex)=>{
      const sheet=workbook.addWorksheet(`${groupIndex+1}_${safeName(group.importerCode)}_${safeName(group.siteCode)}`.replace(/[\[\]]/g,"_").slice(0,31),{
        views:[{state:"frozen",ySplit:8,showGridLines:false}],pageSetup:{paperSize:9,orientation:"landscape",fitToPage:true,fitToWidth:1,fitToHeight:0,margins:{left:0.3,right:0.3,top:0.3,bottom:0.3,header:0.1,footer:0.1}}
      });
      sheet.columns=columns.map(c=>({width:c[1]*0.5}));sheet.pageSetup.printTitlesRow="1:8";
      ...
      merged(4,1,14,`納品先 ${options.destination||""}`);sheet.getRow(4).height=22;
      merged(5,1,14,`連絡先 ${options.contact||""}`);sheet.getRow(5).height=22;
      merged(6,1,14,options.note||"");sheet.getRow(6).height=Math.max(18,Math.ceil(text(options.note).length/100)*14);
      sheet.getRow(7).height=6;
      columns.forEach((col,i)=>{const cell=sheet.getCell(8,i+1);cell.value=col[0];cell.font={name:"Yu Gothic",size:9,bold:true};cell.fill={type:"pattern",pattern:"solid",fgColor:{argb:"FFECEFF1"}};cell.alignment={vertical:"middle",horizontal:"center",wrapText:true}});sheet.getRow(8).height=28;
      let cursor=9,previous="";
